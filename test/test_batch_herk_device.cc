...
void test_batch_herk_device_work( Params& params, bool run )
{
    ...
    std::vector<real_t>     alpha(1, alpha_);
    ...
    blas::batch::herk( layout, uplo, trans, n, k, alpha, dAarray, lda, beta, dCarray, ldc,
                       batch, info, queue );
    queue.sync();
    time = get_wtime() - time;
...
void test_batch_herk_device( Params& params, bool run )
{
    switch (params.datatype()) {
        case libtest::DataType::Integer:
            //test_batch_herk_device_work< int64_t >( params, run );
            throw std::exception();
            break;

        case libtest::DataType::Single:
            test_batch_herk_device_work< float, float >( params, run );
            break;

        case libtest::DataType::Double:
            test_batch_herk_device_work< double, double >( params, run );
            break;

        case libtest::DataType::SingleComplex:
            test_batch_herk_device_work< std::complex<float>, std::complex<float> >
                ( params, run );
            break;

        case libtest::DataType::DoubleComplex:
            test_batch_herk_device_work< std::complex<double>, std::complex<double> >
                ( params, run );
            break;
    }
}
