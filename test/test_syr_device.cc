...
void test_syr_device_work( Params& params, bool run )
{
    ...
    // test error exits
    assert_throw( blas::syr( Layout(0), uplo,     n, alpha, dx, incx, dA, lda, queue ), blas::Error );
    assert_throw( blas::syr( layout,    Uplo(0),  n, alpha, dx, incx, dA, lda, queue ), blas::Error );
    assert_throw( blas::syr( layout,    uplo,    -1, alpha, dx, incx, dA, lda, queue ), blas::Error );
    assert_throw( blas::syr( layout,    uplo,     n, alpha, dx,    0, dA, lda, queue ), blas::Error );
    assert_throw( blas::syr( layout,    uplo,     n, alpha, dx, incx, dA, n-1, queue ), blas::Error );
    ...
    blas::syr( layout, uplo, n, alpha, dx, incx, dA, lda, queue );
    queue.sync();
    time = get_wtime() - time;
...
void test_syr_device( Params& params, bool run )
{
    switch (params.datatype()) {
        case testsweeper::DataType::Single:
            test_syr_device_work< float, float >( params, run );
            break;

        case testsweeper::DataType::Double:
            test_syr_device_work< double, double >( params, run );
            break;

        case testsweeper::DataType::SingleComplex:
            test_syr_device_work< std::complex<float>, std::complex<float> >( params, run );
            break;

        case testsweeper::DataType::DoubleComplex:
            test_syr_device_work< std::complex<double>, std::complex<double> >( params, run );
            break;
