...
void test_batch_symm_device_work( Params& params, bool run )
{
    ...
    if (layout == Layout::RowMajor)
        std::swap( Cm, Cn );
    int64_t lda_ = max( roundup( An, align ), 1 );
    int64_t ldb_ = max( roundup( Cm, align ), 1 );
    ...
    blas::batch::symm( layout, side, uplo, m, n, alpha, dAarray, lda, dBarray, ldb, beta, dCarray, ldc,
                       batch, info, queue);
    time = get_wtime() - time;
...
void test_batch_symm_device( Params& params, bool run )
{
    switch (params.datatype()) {
        case testsweeper::DataType::Single:
            test_batch_symm_device_work< float, float, float >( params, run );
            break;

        case testsweeper::DataType::Double:
            test_batch_symm_device_work< double, double, double >( params, run );
            break;

        case testsweeper::DataType::SingleComplex:
            test_batch_symm_device_work< std::complex<float>, std::complex<float>,
                            std::complex<float> >( params, run );
            break;

        case testsweeper::DataType::DoubleComplex:
            test_batch_symm_device_work< std::complex<double>, std::complex<double>,
