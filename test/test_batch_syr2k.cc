...
void test_batch_syr2k_work( Params& params, bool run )
{
    ...
    blas::batch::syr2k( uplo, trans, n, k, alpha, Aarray, lda, Barray, ldb, beta, Carray, ldc, 
                        batch, info );
    time = get_wtime() - time;
...
void test_batch_syr2k( Params& params, bool run )
{
    switch (params.datatype.value()) {
        case libtest::DataType::Integer:
            //test_batch_syr2k_work< int64_t >( params, run );
            throw std::exception();
            break;

        case libtest::DataType::Single:
            test_batch_syr2k_work< float, float, float >( params, run );
            break;

        case libtest::DataType::Double:
            test_batch_syr2k_work< double, double, double >( params, run );
            break;

        case libtest::DataType::SingleComplex:
            test_batch_syr2k_work< std::complex<float>, std::complex<float>,
                             std::complex<float> >( params, run );
            break;

        case libtest::DataType::DoubleComplex:
            test_batch_syr2k_work< std::complex<double>, std::complex<double>,
